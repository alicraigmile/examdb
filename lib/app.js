...
class examDbDate extends Date {
    ...
    examDbShortDateString() { 
        let date = this,
        dateString = date.toISOString().slice(0,10).replace(/-/g, '/');
        return dateString;
    }
...
app.get('/pathways/:pathway/calendar/:week',function (req, res) {
    ...
    if (! pathway ) {
        ...
        // the dates of the 5 days starting on the 1 of this week 
        var weekDays = [1,2,3,4,5].map(offset => {
            let date = new examDbDate(today);
            date.setDate(date.getDate() - dayOfWeek + offset); 
            return date;
        });

        let dates = weekDays.map(date => date.examDbShortDateString());
