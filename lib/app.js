...
var qualifications = [
    	{id:'gcse', name:'GCSE'},
    	{id:'gce', name:'GCE'},
    	{id:'national5', name:'National 5'},
    	{id:'higher', name:'Higher'},
    	{id:'advancedhigher', name:'Advanced Higher'}
    ],
    providers = [
...
function importRecord(record) {
    ...
    var qualification_name = record['Qualification'],
        qualification_id = qualification_name.toLowerCase();
    ...
    var course_name_raw = record['Course'],
        ...
        course_stub = course_name_raw.toLowerCase().replace(/[^a-z]/ig, '-'),
...
app.get('/pathways/:pathway/calendar/:week',function (req, res) {
    ...
    if (! pathway ) {
        ...
        var days = _.map(dates, function(date) {
            var morningExams = _.filter(exams, {date:date, timeofday:'Morning'}),
                afternoonExams = _.filter(exams, {date:date, timeofday:'Afternoon'});
...
app.post('/exams/upload',function (req, res) {
    ...
    if (file.mimetype != 'text/csv')
        return res.status(400).json({status: 422, message: 'unprocessable entity; expected mimetype text/csv'});

    if (file.truncated)
        return res.status(400).json({status: 422, message: 'unprocessable entity; too large'});

    var records = parse(file.data, {columns:true}, function(err, records) {

        if (err) {
            res.status(400).json({status: 422, message: 'unprocessable entity; can\'t read CSV data'});
        } else {
