...
app.get('/qualifications/:qualification',function (req, res) {
    ...
    if ( ! qual )
	   return res.error.text(404,'qualification not found');
...
app.get('/pathways/:pathway',function (req, res) {
    ...
    if (! pathway )
         return res.error.text(404, 'pathway not found');
...
app.get('/pathways/:pathway/calendar.json',function (req, res) {
    res.error.json(501, 'not implemented');
});

app.get('/pathways/:pathway/calendar.ics',function (req, res) {
    res.error.text(501, 'not implemented');
});
...
app.get('/courses/:course.json',function (req, res) {
    ...
    if ( ! course )
        return res.error.json(404, 'course not found');
...
app.get('/courses/:course',function (req, res) {
    ...
    if ( ! course )
        res.error.text(404, 'course not found');
...
app.get('/programmesofstudy/:programmeofstudy.json',function (req, res) {
    ...
    if (! programmeofstudy)
        return res.error.json(404,'programme of study not found');
...
app.get('/programmesofstudy/:programmeofstudy',function (req, res) {
    ...
    if (! programmeofstudy)
        return res.error.text(404, 'programme of study not found');
...
app.get('/exams.json',function (req, res) {
    store.allExams()
        ...
        .catch(function() {
            res.error.json(500, 'error fetching exams data');
        });
});
...
app.post('/exams/upload',function (req, res) {
    ...
    if (! file)
        return res.error.html(422, 'unprocessable entity; did you remember to upload the file?', template);

    if (file.mimetype != 'text/csv')
        return res.error.html(422, 'unprocessable entity; expected mimetype text/csv', template);

    if (file.truncated)
        return res.error.html(422, 'unprocessable entity; too large', template);

    var records = parse(file.data, {columns:true}, function(err, records) {

        if (err)
            return res.error.html(422, 'unprocessable entity; can\'t read CSV data', template);
...
app.get('/exams/:exam.json',function (req, res) {
    ...
    if ( ! exam )
        return res.error.json(404, 'exam not found');
...
app.get('/exams/:exam',function (req, res) {
    ...
    if ( ! exam )
        return res.error.text(404, 'exam not found');
...
app.get('/examboards/:examboard.json',function (req, res) {
    ...
    if ( ! examboard )
        return res.error.json(404,'examboard not found');
...
app.get('/examboards/:examboard',function (req, res) {
    ...
    if ( ! examboard )
       return res.error.text(404,'examboard not found');
