...
app.get('/search',function (req, res) {
    ...
    let today = new Date();
    if (query.match(todayRegex)) {
        let dateString = today.toISOString().slice(0,10).replace(/-/g, '/');
        query = dateString;
    } else if (query.match(tomorrowRegex)) {
        let tomorrow = new Date(today); // clone today
        tomorrow.setDate(tomorrow.getDate() + 1); //add 1 day
        let dateString = tomorrow.toISOString().slice(0,10).replace(/-/g, '/'); // format: yyyy/mm/dd
        query = dateString;
    }
