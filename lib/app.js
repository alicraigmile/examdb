...
function loadSampleData (store) {
    ...
    _.each([
        {id:'gcse-aqa-mathematics', provider: 'gcse-aqa',rogrammeofstudy:'gcse-mathematics', bitesize:'z8sg6fr'},
...
app.get('/exams.json',function (req, res) {
    const
        outputJSON = (exams) => { res.json(exams) },
        onError = () =>{ res.error.json(500, 'Cannot fetch exams data.') };

    let fetchExams = store.allExams();
...
app.get('/exams',function (req, res) {
    const
        ...
        onError = () =>{ res.render(template, {}); };

    let fetchQualifications = store.allQualifications();
