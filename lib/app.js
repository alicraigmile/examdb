...
const express = require('express'),
    ...
    Store = require('./' + storageEngine + 'store'), //memorystore, postgresstore
...
async function loadSampleData (store) {
    var promises = [],
        pushPromise = promise => promises.push(promise);
    ...
    _.chain([
        ...
        .map(store.addQualification, store)
        .map(pushPromise);
    ...
    _.chain([
        ...
        .map(store.addProgrammeOfStudy, store)
        .map(pushPromise);
...
console.log('loadSampleData complete.')
...
app.get('/qualifications.json',function (req, res, next) {
    let renderJSON = qualifications => res.json(qualifications),
        renderSoftwareError = error => res.error.json(500, 'Could not retreive qualifications');

    store.allQualifications()
        .then(renderJSON)
        .catch(renderSoftwareError)
        .catch(next);
});
...
app.get('/qualifications/:qualification',function (req, res, next) {
    var qualificationId = req.params.qualification,
        inflateExamboard = async p => { console.log(`inflate: ${p.examboard}`); p.e = await store.examboard(p.examboard); console.log(`inflated: ${p.e.name}`);  return p }, //p = 'provider'
        fetchProviders = qualification => {
            console.log(qualification);
            return store.providersOfQualification(qualification.id)
                .then(function (providers) {
                    console.log(providers);
                    let output = {
                        qualification:qualification,
                        providers:_.chain(providers).map(inflateExamboard).value()
                    };
        ...
        renderNotFoundError = error => { res.error.html(404, `Qualification '${qualificationId}' was not found: ${error}`) };
        
    store.qualification(qualificationId)
        .then(fetchProviders)
        .catch(renderNotFoundError)
        .catch(next);

});

app.get('/pathways.json',function (req, res, next) {
    let renderJSON = providers => res.json(providers),
        renderSoftwareError = error => res.error.json(500, 'Could not retreive pathways');
...
app.get('/pathways/:provider',function (req, res) {
    var providerId = req.params.provider,
        ...
        courses,
        output,
        orderByPosName = course => course.pos.name,
        inflateProgrammeOfStudy = async course => {course.pos = await store.programmeOfStudy(course.programmeofstudy); return course};
